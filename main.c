...
int main(int argc, char *argv[]) {
  ...
  // {
  //     printf("Value %d: %s\n", i, argv[i]);
  //     printf("Equal to null? %d\n", argv[i] == NULL); // boolean, char, short
  //     -> int when called
  //                                                     // in variadic
  //                                                     variables.
  // }
  ...
  argv += optind; // advance using pointer arithmetic the argv pointer to point
                  // to the optind index.

  if (argc != 1)
    usage("Expected only 1 positional argument: file\n");
  ...
  while (!ferror(fp) && !feof(fp)) {
    ...
    // We are in a newline and hit a character which isnt part of an ignore
    // sequence we leave the newline state.
    if (newline && !isspace(ch))
      newline = false;
  ...
  charc--; // we count an extra character.
